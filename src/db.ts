...
function createSchema(database: Database.Database): void {
  ...
  // Add system_instruction column if it doesn't exist
  try {
    database.exec(
      `ALTER TABLE registered_groups ADD COLUMN system_instruction TEXT`,
    );
  } catch {
    /* column already exists */
  }

  // Add ephemeral column if it doesn't exist
  try {
    database.exec(
      `ALTER TABLE registered_groups ADD COLUMN ephemeral INTEGER DEFAULT 0`,
    );
  } catch {
    /* column already exists */
  }
  ...
  try {
    ...
    database.exec(
      `UPDATE chats SET channel = 'discord', is_group = 1 WHERE jid LIKE 'discord-%'`,
    );
    database.exec(
...
/**
 * Store a message directly (used for non-Baileys message formats).
 */
...
export function getNewMessages(
  ...
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, is_bot_message, is_from_me
    FROM messages
...
export function getMessagesSince(
  ...
  `;
  const rows = db.prepare(sql).all(chatJid, sinceTimestamp, limit) as any[];
...
export function updateTaskAfterRun(
  ...
  // Remove from queued persistence once run is finished
  removeQueuedTask(id);
}
...
export function addQueuedTask(taskId: string, groupJid: string): void {
  db.prepare(
    'INSERT OR IGNORE INTO queued_tasks (task_id, group_jid) VALUES (?, ?)',
  ).run(taskId, groupJid);
}

export function removeQueuedTask(taskId: string): void {
  db.prepare('DELETE FROM queued_tasks WHERE task_id = ?').run(taskId);
}

export function getAllQueuedTasks(): { task_id: string; group_jid: string }[] {
  return db.prepare('SELECT task_id, group_jid FROM queued_tasks').all() as {
    task_id: string;
    group_jid: string;
  }[];
}
...
export function deleteRegisteredGroup(jid: string): void {
  // Never delete the main group
  const row = db.prepare('SELECT folder FROM registered_groups WHERE jid = ?').get(jid) as { folder: string } | undefined;
  if (row?.folder === 'main') {
